...
import joblib
from sklearn.linear_model import Ridge, Lasso, ElasticNet, LarsCV, GammaRegressor, BayesianRidge, PoissonRegressor, TweedieRegressor
from sklearn.model_selection import train_test_split
...
input_current = 0.03 #we only want to look at one current input level so the model doesn't have to learn dependence on that in addition to the morphology and trans stuff
...
#approved_input_features = ['num_branches', 'num_nodes', 'num_tips', 'total_length', 'total_surface_area', 'total_volume', 'Ccnl1', 'Cnot6', 'Gigyf1', 'Ppp4r4', 'Rpl17', 'Slc38a6']
...
input_data_path = 'F:\Big_MET_data\PCAd_morph_trans.csv'
...
for input_current in currents_to_check:
	...
	#for data_idx in range(20):
		...
		if len(output_features) > 0:
			...
			this_input_data = morpho_trans_data.iloc[data_idx, 5:30].to_numpy().astype(np.float64)
	...
	ridge = Ridge(max_iter=my_max_iter, solver = 'sag')  # alpha is the regularization strength
	ridge.fit(X_train, y_train)
	
	ridge_preds = ridge.predict(X_test)
	
	print(input_current, "Ridge R²:", r2_score(y_test, ridge_preds))
	...
	plt.ylabel('True')
	plt.title(f'Ridge Predictions {input_current}')
	plt.legend()
	plt.show()