...
def make_cell(swc, fit):
    ...
    decor.place('"midpoint"', A.iclamp(0.2 * U.s, 0.8 * U.s, -100 * U.pA), "ic0")
    decor.place('"midpoint"', A.iclamp(1.2 * U.s, 0.8 * U.s, 100 * U.pA), "ic1")
    decor.place('"midpoint"', A.iclamp(2.2 * U.s, 0.8 * U.s, 200 * U.pA), "ic2")
...
def run_simulation(task):
    ...
    try:
        ...
        # Run simulation
        model.run(tfinal=3.2 * U.s, dt=5 * U.us)
        ...
        csv_path = f"/mnt/f/arbor_ubuntu/10k_ephys/{folder_name}_{cell_name}_{counter}.csv"
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(folder):
        for file in files:
            morph_path = os.path.join(root, file)
            fit_params_path = f"/mnt/f/arbor_ubuntu/10k_randomized_jsons/random_genome_{counter}.json"
            folder_name = root.split("/")[-2]
            cell_name = file.split(".")[-3]
            tasks.append((morph_path, fit_params_path, folder_name, cell_name, counter))
            counter += 1

    # Run in parallel (all cores)
