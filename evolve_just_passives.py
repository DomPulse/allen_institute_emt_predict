...
#allen institute fitting paper https://www.nature.com/articles/s41467-017-02718-3
...
def mutate(baby_param, baby_cap):
	baby_param[7, 0] = np.max([np.random.normal(baby_param[7, 0], 0.01 + np.abs(baby_param[7, 0])/10), 0])
	baby_param[7, 1] = np.random.normal(baby_param[7, 1], np.abs(baby_param[7, 1])/10)
	baby_cap = np.max([np.random.normal(baby_cap, baby_cap/10), 0])
	return baby_param, baby_cap

def death_and_sex(current_params, current_caps, current_error):
	...
	for n in range(num_neurons):
		...
		new_caps[n] = (current_caps[mommy_idx] + current_caps[daddy_idx])/2
		new_params[n], new_caps[n] = mutate(new_params[n], current_caps[n])
	
	return new_params, current_caps
...
for i in range(len(all_times)):
	...
	these_derived = derived_ephys_props(all_volts[i, :stop_at], ephys_time_step)
	
	num_spikes = these_derived[3]
	
	right_answers.append(these_derived)
...
AReverse = -80
HReverse = -20
LeakReverse = np.random.normal(-90, 5, num_neurons)
...
for n in range(num_neurons):
	all_params[n, :, 0] = [Na_g, CaT_g, CaS_g, A_g, KCa_g, Kd_g, H_g, Leak_g*np.random.rand()]
...
for e in range(epochs):
	...
	train_on = np.random.choice(allowed_idx) #picks a random ephys experiment to serve as dataset
	...
	all_params, caps = death_and_sex(all_params, caps, all_errors)
